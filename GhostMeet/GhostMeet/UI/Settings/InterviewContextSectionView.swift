...
struct InterviewContextSectionView: View {
    ...
    private func placeholder(for field: InterviewContext.Field) -> String {
        switch field {
        case .stories:
            String(localized: "По одной истории на абзац: ситуация — задача — что сделал — результат в цифрах. Например: платежи падали по ночам → нашёл дедлок в очереди → переписал на идемпотентные ретраи → отказы с 3% до 0.1%")
        case .motivation:
            String(localized: "Например: продукт про платежи, знаком по прошлой работе; команда 12 человек; интересна миграция на Go")
        case .compensation:
            String(localized: "Например: 350–400 тысяч на руки, готов обсуждать при сильном опционе")
        case .questions:
            String(localized: "Например: как устроен онбординг; кто принимает решение по архитектуре; почему открыта вакансия")
        }
    }
