...
**Третий раз — аккорд, нажатый в другом экземпляре.** Хост зарегистрировал глобальные хоткеи Carbon, владелец в это время тестировал установленную копию и нажал ⌥⌘K — событие пришло в хост, `CarbonHotkeyRegistry.deliver(id:)` упал в `Dictionary.lookup` на уже разрушенном реестре, и прогон отчитался «0 tests in 65 suites». Ни одна правка в дереве к этому отношения не имела; повторный прогон дал 835 зелёных. Отсюда: **пока идёт `xcodebuild test`, глобальные аккорды не нажимать** — и `~/Library/Logs/DiagnosticReports/GhostMeet-*.ips` называет виновника за минуту, а поиск по своим правкам — за час.
