...
**[docs/lessons-from-live-runs.md](docs/lessons-from-live-runs.md) — то, что уже ломалось вживую**, по слоям: промпты и жанры, транскрипт и нажатие, поток ответа и провайдеры. Читать по слою, который трогаешь, а не целиком. Цена незнания измерена и одинакова во всех трёх: ответ выходит связным, по теме и не про то, что спросили, — а отличить это от «модель не поняла вопрос» пользователь не может.
