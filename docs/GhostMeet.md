...
- **Уведомление о новой версии** — одна строка под шапкой, появляется, только если опубликованная версия новее установленной, и исчезает, как только включили прослушивание ([ADR-0010](adr/0010-update-check-at-launch.md)). Приложение раздаётся образом и себя не обновляет, поэтому без этой строки машина остаётся на выданной когда-то сборке навсегда. Ведёт на страницу релиза, а не на скачивание; всякий сбой проверки — молчание
...
│   │                                 # UpdateNoticeView — строка о новой версии, до звонка
...
└── Utilities/                        # часы, Keychain, геометрия окна
    ...
    └── UpdateCheck.swift             # есть ли версия новее; ReleaseCatalog как шов
```
